                    ...
                    <div className="flex flex-col lg:flex-row gap-8 h-[800px]">
                        ...
                        <div className="flex-1 bg-white rounded-3xl border border-border/50 shadow-float relative overflow-hidden flex items-center justify-center">
                            <div className="text-center space-y-4">
                                ...
                                <p className="text-muted-foreground">Interactive 3D preview would render here.</p>
                            </div>

                            {/* Tool Overlays */}
                            <div className="absolute left-6 top-1/2 -translate-y-1/2 space-y-4">
                                {[Layout, Paintbrush, Cog].map((Icon, idx) => (
                                    <button key={idx} className="w-12 h-12 rounded-xl bg-white shadow-soft border border-border/50 flex items-center justify-center hover:bg-primary/5 transition-colors">
                        ...
                        <div className="w-full lg:w-96 bg-card rounded-3xl border border-border/50 p-8 space-y-10 overflow-y-auto">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    {['L', 'W', 'H'].map((dim) => (
                                        <div key={dim} className="space-y-2">
                                            ...
                                            <input type="number" defaultValue={20} className="w-full bg-muted/30 border border-border/50 rounded-lg px-2 py-1 outline-none" />
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="pt-8 border-t border-border/50">
                                <div className="flex justify-between items-end mb-6">
                                    <div className="text-sm text-muted-foreground">Estimated Unit Price</div>
