...
import { Star, MessageSquare, Heart, Zap, Globe, Mail, ExternalLink, ShieldCheck, Loader2 } from "lucide-react";
...
export const SupplierActionCenter = ({ supplier }: SupplierActionCenterProps) => {
            ...
            >
                ...
                <div className="space-y-8">
                    <div className="flex items-center justify-between px-2">
                        <div className="space-y-1">
                            <span className="text-[9px] font-black text-primary tracking-[0.2em] uppercase">PEOPLE SCORE</span>
                            <div className="flex items-center gap-2">
                                <div className="flex text-primary">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} className={`w-4 h-4 ${i < Math.floor(supplier.rating) ? 'fill-primary' : 'opacity-20'}`} />
                                    ))}
                                </div>
                                <span className="font-black text-xl tracking-tighter">{supplier.rating}</span>
                            </div>
                        </div>
                        <div className="text-right space-y-1">
                            <span className="text-[9px] font-black text-muted-foreground tracking-[0.2em] uppercase">VERIFIED</span>
                            <div className="font-black text-lg tracking-tighter">{supplier.reviewCount}+</div>
                        </div>
                    </div>
                ...
                <div className="space-y-6 pt-8 border-t border-border/20">
                    {supplier.website && (
                        <div className="flex items-center gap-4 group/item">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="text-[9px] font-black text-muted-foreground tracking-[0.15em] mb-1 uppercase">CONNECT ONLINE</div>
                                <a href={supplier.website} target="_blank" className="font-bold text-sm text-foreground hover:text-primary transition-colors flex items-center gap-1.5 truncate">
                                    <span className="truncate">{supplier.website.replace('https://', '').replace('http://', '')}</span>
                    ...
                    {supplier.email && (
                        <div className="flex items-center gap-4 group/item">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="text-[9px] font-black text-muted-foreground tracking-[0.15em] mb-1 uppercase">SECURE MESSAGE</div>
                                <a href={`mailto:${supplier.email}`} className="font-bold text-sm text-foreground hover:text-primary transition-colors flex items-center gap-1.5 truncate">
                ...
                >
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="font-black text-[10px] tracking-tight">Kumo Secure Shield™</span>
                    </div>
