...
export const SupplierCapabilities = ({ features }: SupplierCapabilitiesProps) => {
    return (
            ...
            <AnimatePresence>
                {isHovered && (
                    <motion.div
                        ...
                        className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 z-50 w-64 p-4 rounded-2xl bg-foreground text-background shadow-2xl pointer-events-none"
                    >
                        <div className="space-y-1">
                            <p className="text-xs font-black uppercase tracking-widest text-primary/80">{feature.title}</p>
                            <p className="text-sm font-medium leading-relaxed">{feature.description}</p>
                        </div>
