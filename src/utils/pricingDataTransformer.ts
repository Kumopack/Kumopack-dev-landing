...
export const transformApiDataToPricingInfo = (
  apiData: any[],
  type: PricingType,
): Record<string, TransformedPricingPlan> => {
  ...
  const groupedData = apiData.reduce((acc: any, item: PricingItem) => {
    ...
    const isMonthlyBuyer =
      item.monthMultiplier === 1 && item.baseSlug?.includes("months");
    const isAnnualBuyer =
      item.monthMultiplier === 1 && item.baseSlug?.includes("year");

    const isMonthlySupplier =
      item.monthMultiplier === 1 && item.durationDay === 30;
    const isAnnualSupplier =
      item.monthMultiplier === 12 && item.durationDay === 365;
  ...
  const allKeys = [
    ...orderedKeys,
    ...Object.keys(groupedData).filter((key) => !orderedKeys.includes(key)),
  ];
  ...
  allKeys.forEach((slug) => {
    ...
    if (monthlyItem || annualItem) {
      ...
      const monthlyPrice = monthlyItem
        ? monthlyItem.displayPrice || monthlyItem.price || 0
        : 0;
      const annualPrice = annualItem
        ? annualItem.displayPrice || annualItem.price || 0
        : 0;
